...
def load_plugins(plugin_name):
    path = Path(f"Oxyspam/modules/{plugin_name}.py")
    spec = importlib.util.spec_from_file_location(f"Oxyspam.modules.{plugin_name}", path)
    load = importlib.util.module_from_spec(spec)
    load.logger = logging.getLogger(plugin_name)
    spec.loader.exec_module(load)
    sys.modules["Oxyspam.modules." + plugin_name] = load
    print("𝗢𝗫𝗬𝗚𝗘𝗡 𝗛𝗔𝗦 𝗜𝗠𝗣𝗢𝗥𝗧𝗘𝗗 " + plugin_name)


files = glob.glob("Oxyspam/modules/*.py")
for name in files:
    with open(name) as a:
        patt = Path(a.name)
